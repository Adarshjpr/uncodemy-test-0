/**
 * Curriculum Accordion — Dynamic Renderer
 * -----------------------------------------------------------------
 * CSS/classes bilkul same rakhe gaye hain (jaisa original HTML me the).
 * Structure ab CURRICULUM_DATA se generate hota hai.
 *
 * NAYA (is update me):
 *   1. Ye script sirf un pages par render hoga jinke URL me
 *      "data-analytics" ya "data-analyst" ho (dono ko same treat
 *      kiya gaya hai) — e.g. /course/data-analytics-training-course-in-noida
 *   2. URL ke aakhri hisse (…-in-<city>) se city nikal li jaati hai,
 *      first letter capital karke title me "Courses in <City>" ban
 *      jaata hai. City na mile to default "Delhi" use hota hai.
 *   3. Naye modules add kiye gaye hain: Generative AI topic (AI module
 *      me), R Programming, ggplot2, Looker, MATLAB — aur Excel/Tableau
 *      modules me extra topic cards add kiye gaye hain.
 *
 * Usage:
 *   1. Container me ye markup rakho:
 *      <section class="syllabus-bottom" id="curriculum"></section>
 *   2. Is file ko include karo — auto-init DOMContentLoaded pe already ho jaata hai.
 * -----------------------------------------------------------------
 */
...
(function () {
  ...
  // ------------------------------------------------------------------
  // 0. URL-BASED CONTEXT DETECTION (course type + city)
  // ------------------------------------------------------------------
  function detectPageContext() {
    const path = (window.location && window.location.pathname ? window.location.pathname : "").toLowerCase();

    // "data-analytics" aur "data-analyst" dono ko same course treat karo
    const isDataAnalyticsPage = /data-analytics|data-analyst/.test(path);

    // URL ke end se city nikalo, jaise: ...-training-course-in-noida -> "noida"
    let city = "Delhi"; // default fallback
    const cityMatch = path.match(/-in-([a-z0-9-]+)\/?$/);
  ...
  // ------------------------------------------------------------------
  // 1. DATA — yahi ek jagah edit karni hai naya module/topic add karne ke liye
  // ------------------------------------------------------------------
      ...
      modules: [
        {
          ...
          topics: [
            ...
            {
              title: "Data Structure and File Operations",
              points: [
            ...
            {
              title: "Working with Modules and Handling Exceptions",
              points: [
                "Standard Libraries",
        ...
        },
        {
          ...
          topics: [
            ...
            },
            {
              ...
              points: ["Black box", "SVM hyperplane", "Max margin hyperplane", "Kernel tricks for non linear spaces"],
            },
            {
        ...
        },
        {
          ...
          topics: [
            {
              title: "Getting Started and Creating, Selecting &amp; Retrieving Data with SQL",
              points: [
            ...
            {
              title: "Filtering, Sorting, and Calculating Data with SQL",
              points: [
            ...
            {
              title: "Subqueries and Joins in SQL",
              points: [
                "Using Subqueries",
            ...
            {
              title: "Modifying and Analyzing Data with SQL",
              points: [
            ...
            {
              title: "Accessing Databases using Python",
              points: [
        ...
        },
        {
          ...
          topics: [
            {
              title: "Analyzing and Visualizing Data using Excel",
              points: [
        ...
        },
        {
          ...
          topics: [
            {
              title: "Analyzing and Visualizing Data using Tableau",
              points: [
        ...
        },
        {
          ...
          topics: [
            ...
            {
              title: "Creating Power BI Reports, Auto Filters",
              points: [
        ...
        // ========== MODULE 8: ARTIFICIAL INTELLIGENCE (AI) & GENERATIVE AI ==========
        {
          ...
          topics: [
            ...
            {
              title: "Prompt Engineering & AI Tools for Analysts",
              points: [
            ...
            {
              title: "Natural Language Processing (NLP) and LLMs",
              points: [
                "Introduction to NLP",
            ...
            {
              title: "AI Implementation and Deployment",
              points: [
            ...
            {
              title: "Responsible AI and Future Trends",
              points: [
        ...
        // ========== MODULE 11: BUSINESS INTELLIGENCE WITH LOOKER ==========
        {
          heading: "11. Business Intelligence with Looker",
          topics: [
            {
              title: "Business Intelligence using Looker",
              points: [
        ...
        // ========== MODULE 12: MATLAB FOR DATA ANALYSIS ==========
        {
          heading: "12. MATLAB for Data Analysis",
          topics: [
            {
              title: "MATLAB for Data Analysis",
              points: [
  ...
  /** Global running counter for unique heading/collapse IDs (matches original numbering style) */
  let idCounter = 8; // pehla topic id "9" se start hoga jaisa original me tha
  ...
  function buildTopicCard(topic, accordionId) {
    const id = nextId();
  ...
  function buildModule(module, accordionId) {
    const topicCards = module.topics.map((t) => buildTopicCard(t, accordionId)).join("\n");
    return `
  ...
  // ------------------------------------------------------------------
  function renderCurriculum(targetSelector = null) {
    const { isDataAnalyticsPage, city } = detectPageContext();

    // Ye curriculum sirf data-analytics / data-analyst URL par hi render hoga
    if (!isDataAnalyticsPage) {
      console.warn(
        "[curriculum-dynamic] Skipped: URL me 'data-analytics' ya 'data-analyst' nahi mila, is page ke liye curriculum render nahi hoga."
      );
      return;
    }

    const data = buildCurriculumData(city);
    idCounter = 8; // reset counter har render pe

    const modulesHtml = data.modules.map((m) => buildModule(m, data.accordionId)).join("\n");
    ...
    const target =
      ...
      document.getElementById(data.sectionId) ||
      document.querySelector(`.syllabus-bottom#${data.sectionId}`);

    if (!target) {
      console.warn(
        `[curriculum-dynamic] Target section not found. Add <section class="syllabus-bottom" id="${data.sectionId}"></section> to your page.`
      );
      return;
    }

    target.classList.add("syllabus-bottom");
    target.id = data.sectionId;
  ...
  // Expose for manual re-render / debugging
  window.CurriculumRenderer = {
    render: renderCurriculum,
    detectPageContext: detectPageContext,
    getData: buildCurriculumData,
  };
})();