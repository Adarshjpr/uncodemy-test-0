...
//     if (isset($data['name'], $data['email'], $data['phone'], $data['location'], $data['program'])) {
...
//         $program = mysqli_real_escape_string($connection, $data['program']);

//         // Insert data into the database
//         $sql = "INSERT INTO `pg-program-data` (`name`, `email`, `phone`, `location`, `program`, `created_at`) 
//                 VALUES ('$name', '$email', '$phone', '$location', '$program', NOW())";

//         if (mysqli_query($connection, $sql)) {
//             echo json_encode(["status" => "success", "message" => "Form submitted successfully."]);
//         } else {
//             echo json_encode(["status" => "error", "message" => "Error executing query: " . mysqli_error($connection)]);
//         }
//     } else {
...
try {
    ...
    // JSON body (fetch se aa raha hai), fallback $_POST
    ...
    if (empty($name) || empty($email) || empty($phone) || empty($location) || empty($program)) {
    ...
    try {
        // 1. Database insert
        $stmt = $conn->prepare(
            "INSERT INTO `pg-program-data` (`name`, `email`, `phone`, `location`, `program`, `created_at`)
             VALUES (?, ?, ?, ?, ?, NOW())"
        );
        if (!$stmt) {
            throw new Exception("Database prepare failed: " . $conn->error);
        }
        $stmt->bind_param("sssss", $name, $email, $phone, $location, $program);
        ...
        // 2. WhatsApp
        $response['whatsapp'] = sendWhatsAppMessage($conn, $phone, $name, $program);

        // 3. Email (ek retry ke saath)
        $response['email'] = sendUserEmail($email, $name, $program);
        if (!$response['email']) {
...
function sendUserEmail($email, $name, $program) {
    $mail = null;
    try {
        ...
        $mail->Subject = 'Thank You for Contacting Uncodemy - ' . $program;

        $safeName    = htmlspecialchars($name, ENT_QUOTES, 'UTF-8');
        $safeProgram = htmlspecialchars($program, ENT_QUOTES, 'UTF-8');
      ...
      <tr>
        <td style="padding:30px 28px 10px;font-size:15px;color:#222222;line-height:1.6;">
          ...
          <p style="margin:0 0 14px;">
            Thanks for reaching out about our <strong>$safeProgram</strong> program.
...
function sendWhatsAppMessage($conn, $mobile, $name, $program) {
    $queueId = null;
    try {
        ...
        $message  = "Hi *$name*!\n\n";
        $message .= "*Thanks for reaching out about our $program program!*\n\n";
